...
import { PAGE_THEME } from '../Constants/ThemeConstants'
...
export function Sidebar() {
  ...
  return (
    <nav
      className={`sticky top-0 z-50 w-14 ${t.bg}  border-r border-[#e8e8ec]/50 flex flex-col items-center py-4 gap-1.5 shrink-0`}
      aria-label="Main navigation"
    >
      {/* Logo */}
      <Link
        href="/dashboard"
        className={`w-8 h-8 ${t.bg} rounded-lg flex items-center justify-center text-white mb-2.5`}
      ...
      {NAV_ITEMS.map(({ href, label, icon }) => {
        ...
        return (
          <Link
            key={href}
            href={href}
            className={`w-9 h-9 rounded-lg flex items-center justify-center cursor-pointer transition-[background,color] duration-150 ${active
              ? 'bg-[#eff3ff] text-[#1a6bff]'
              : `text-[#9898a8] ${t.bg} hover:bg-[#f0f0f6] hover:text-[#1a1a2e]`
              }`}
            title={label}
            aria-label={label}
