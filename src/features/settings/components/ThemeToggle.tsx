...
import { TASK_THEMES, type ThemeKey } from '@/Common/Constants/ThemeConstants'
...
export function ThemeToggle({ theme: themeProp, onChange: onChangeProp }: ThemeToggleProps) {
  const { theme: contextTheme, setTheme: contextSetTheme } = useTheme()
  const theme = themeProp ?? contextTheme
  const onChange = onChangeProp ?? contextSetTheme
      ...
      {(['light', 'dark'] as const).map((option) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[6px] text-[12px] font-medium cursor-pointer transition-[background,color] duration-150 ${active
                ? theme === 'dark'
